...
function renderTemuVariantRows(){
  ...
  const commonImages=[...new Set(products.map(item=>item.common_image_url).filter(Boolean))];const common=$('#temuCommonImage');common.classList.toggle('empty',!commonImages.length);common.innerHTML=commonImages.length?`<img src="${escapeHtml(commonImages[0])}" alt="" loading="lazy"><div><strong>Közös termékkép</strong><small>${commonImages.length===1?'Egyszer kerül a Temu galériába.':`${commonImages.length} különböző kép van az importban; ellenőrizd a Woo terméket.`}</small></div>`:'<span class="temu-common-image-preview"></span><div><strong>Nincs közös termékkép</strong><small>Állítsd be a WooCommerce termék Általános adatainál.</small></div>';
  renderTemuMappings();
}
...
function previewTemuSelection(){
  ...
  const selectedProducts=selectedGroups.flat();const commonImages=[...new Set(selectedProducts.map(item=>item.common_image_url).filter(Boolean))];const variantImages=[...new Set(selectedProducts.map(item=>item.image_url).filter(Boolean))];const plan={api_method:'bg.local.goods.add',image_upload_method:'bg.local.goods.image.upload',ready_to_publish:false,category:{id:temuSelectedCategory.id,name:temuSelectedCategory.name},family,images:{common:commonImages,variants:variantImages,upload_once:[...new Set([...commonImages,...variantImages])]},variant_rows:selectedGroups.map(group=>({type:group[0].type,color:group[0].color,sizes:group.map(item=>item.size),skus:group.map(item=>item.sku),stock:group.reduce((sum,item)=>sum+Number(item.stock||0),0),image_url:group[0].image_url||''})),sale_mappings:saleMappings,properties};$('#temuSelectionPayload').textContent=JSON.stringify(plan,null,2);$('#temuSelectionWrap').classList.remove('hidden');toast(commonImages.length?'A Temu feltöltési terv a közös képpel elkészült; még nem küldtünk adatot.':'A terv elkészült, de nincs közös Temu-kép beállítva a Woo terméknél.',commonImages.length?'success':'error');
}
