...
function setAllegroEnvironment(environment){
  const production=environment==='production';
  ['#uploadEnvironment','#bulkUploadEnvironment'].forEach(selector=>{
    const chip=$(selector);if(!chip)return;
    chip.textContent=production?'ÉLES':'SANDBOX';
    chip.classList.toggle('production',production);
  });
}
...
async function loadUpload(){
  ...
  try{
    ...
    setAllegroEnvironment(settings.environment);
  }catch(error){toast(`A környezet nem ellenőrizhető: ${error.message}`,'error')}
  try{
    const [products,templates,imports]=await Promise.all([api('/api/products?marketplace=allegro'),api('/api/templates'),api('/api/imports?marketplace=allegro')]);uploadProducts=products.products;uploadTemplates=templates.templates;allegroImportBatches=imports.imports||[];
...
async function inspectCategory(categoryId,template=undefined){
  try{
    ...
    const productId=$('#offerProduct').value;const suffix=productId?`?product_id=${encodeURIComponent(productId)}`:'';const [data,taxData]=await Promise.all([api(`/api/categories/${encodeURIComponent(categoryId)}${suffix}`),api(`/api/tax-settings?category_id=${encodeURIComponent(categoryId)}&country_code=HU`)]);selectedCategory=data.category;renderTaxSettings(taxData);renderCategory(data.category,activeTemplate);$('#categoryInspector').classList.remove('hidden');$('#offerPreviewCard').classList.remove('hidden');$('#categoryInspector').scrollIntoView({behavior:'smooth',block:'start'});return true;
  }catch(error){toast(error.message,'error');return false}
}
function renderTaxSettings(data){
  const settings=data.settings||[];const select=$('#vatRate');select.innerHTML='<option value="">Válassz magyar áfakulcsot…</option>'+settings.map(setting=>`<option value="${escapeHtml(setting.id)}">${escapeHtml(setting.label)}</option>`).join('');if(data.default_id)select.value=String(data.default_id);if(!settings.length)select.innerHTML='<option value="">Nincs elérhető magyar áfakulcs</option>';
}
...
function collectTemplateRules(){
  ...
  [['__shipping_rate__','#shippingRate'],['__handling_time__','#handlingTime'],['__producer__','#responsibleProducer'],['__responsible_person__','#responsiblePerson'],['__safety_information__','#safetyInformation'],['__tax_setting__','#vatRate']].forEach(([parameter,selector])=>rules.push({parameter_id:parameter,mode:'fixed',value:$(selector).value}));rules.push({parameter_id:'__preorder__',mode:'fixed',value:String($('#preorder').checked)});return rules;
}
